    ...
    <div className="p-8 max-w-7xl mx-auto">
      <div className="flex justify-between items-start mb-10">
        <div>
          ...
          <p className="text-slate-500 max-w-2xl text-lg">{t('hr.sub')}</p>
        </div>
          ...
          {state.currentUser?.role === 'admin' && (
            <div className="bg-white px-6 py-3 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-4">
              <div className="text-right">
                <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-0.5">{t('hr.domainWeight')}</div>
        ...
        <div className="lg:col-span-8 bg-white rounded-[2.5rem] p-10 shadow-sm border border-slate-100">
          <div className="flex justify-between items-center mb-10">
            <div>
              ...
              <p className="text-slate-400 text-sm font-bold mt-1">{t('hr.overallPerf').replace('{count}', state.teachers.length.toString())}</p>
            </div>
            <div className="flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl">
              <div className="w-3 h-3 rounded-full bg-indigo-500"></div>
              <span className="text-xs font-black text-slate-500 uppercase tracking-widest">{t('hr.avgScore14')}</span>
            </div>
          </div>
        ...
        <div className="lg:col-span-4 flex flex-col gap-8">
          <div className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl shadow-slate-200 relative overflow-hidden flex-1">
            ...
            <div className="relative z-10">
              <div className="text-[11px] font-black text-slate-400 uppercase tracking-[0.4em] mb-8">{t('hr.sysStats')}</div>
              <div className="grid grid-cols-2 gap-y-10 gap-x-6">
                <div>
                  <div className="text-5xl font-black mb-2" style={{ fontFamily: '"Barlow Condensed", sans-serif' }}>{stats.avg}</div>
                  <div className="text-[11px] font-black text-slate-400 uppercase tracking-widest">{t('hr.avgHrScore')}</div>
                </div>
                <div>
                  <div className="text-5xl font-black mb-2 text-emerald-400" style={{ fontFamily: '"Barlow Condensed", sans-serif' }}>{stats.excellent}</div>
                  <div className="text-[11px] font-black text-slate-400 uppercase tracking-widest">{t('hr.excellent')}</div>
                </div>
                <div>
                  <div className="text-5xl font-black mb-2 text-rose-400" style={{ fontFamily: '"Barlow Condensed", sans-serif' }}>{stats.warning}</div>
                  <div className="text-[11px] font-black text-slate-400 uppercase tracking-widest">{t('hr.needsReview')}</div>
                </div>
                <div>
                  <div className="text-5xl font-black mb-2 text-indigo-400" style={{ fontFamily: '"Barlow Condensed", sans-serif' }}>{chartData.length}</div>
                  <div className="text-[11px] font-black text-slate-400 uppercase tracking-widest">{t('hr.totalStaff')}</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-[3rem] shadow-sm border border-slate-100 overflow-hidden">
        <div className="p-8 border-b border-slate-50 bg-slate-50/30 flex flex-wrap justify-between items-center gap-6">
          <h2 className="text-2xl font-black text-slate-800">{t('hr.staffRecords')}</h2>
          ...
          <div className="text-xs font-black text-slate-400 uppercase tracking-[0.3em]">{t('hr.realtimeSync')}</div>
        </div>
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-50">
            {filteredTeachers.map(tData => {
              ...
              return (
                <tr key={tData.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="py-6 px-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-400 font-black text-xs shadow-inner">
                        {tData.fullName.split(' ').map(n => n[0]).join('')}
                      </div>
                      <div>
                        <div className="font-black text-slate-900 text-base">{tData.fullName}</div>
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{tData.role} · {tData.subject}</div>
                        {tData.employeeId && <div className="text-[9px] font-mono font-bold text-indigo-400 mt-1">ID: {tData.employeeId}</div>}
                      </div>
                    </div>
                  </td>
                  <td className="py-6 px-4 text-center">
                    <div className="inline-block">
                      <input 
                        ...
                        onChange={e => handleDataChange(tData.id, 'absences', e.target.value)}
                        disabled={isReadOnly}
                      ...
                      }}>{t('hr.score').replace('{score}', s1.toString())}</div>
                    </div>
                  </td>
                  <td className="py-6 px-4 text-center">
                    <div className="inline-block">
                  ...
                  <td className="py-6 px-6">
                    <textarea 
                      value={data.notes || ''} 
                      onChange={e => handleDataChange(tData.id, 'notes', e.target.value)}
                      disabled={isReadOnly}
                      placeholder={t('hr.addObservations')}
